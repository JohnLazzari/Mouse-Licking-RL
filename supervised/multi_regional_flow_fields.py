...
def main():
    ...
    x_data, len_seq = gather_inp_data(DT, HID_DIM)
    ...
    xn = torch.zeros(size=(1, 1, total_num_units)).cuda()
    inhib_stim = torch.zeros(size=(1, max(len_seq), hn.shape[-1]), device="cuda")

    # Get original trajectory
    with torch.no_grad():
        _, _, act = rnn(iti_inp[CONDITION:CONDITION+1], cue_inp[CONDITION:CONDITION+1], hn, xn, inhib_stim, noise=False)
    ...
    flow_field.fit_pca(sampled_acts[:len_seq[CONDITION], region_start:region_end])
