...
class FixedPointFinderTorch(FixedPointFinderBase):
    ...
    def _run_joint_optimization(self, initial_states, inputs, region_start, region_end, cond_ids=None):
        ...
        inhib_stim = torch.zeros_like(x_1xbxd)
        ...
        while True:
            ...
            F_x_1xbxd, F_x_bx1xd  = self.rnn(inputs_bx1xd, x_1xbxd, inhib_stim)

            dx_bxd = torch.squeeze(x_1xbxd[:, :, region_start:region_end] - F_x_1xbxd[:, :, region_start:region_end])
            ...
            # Manually zero out gradients for neurons outside of region
            if x_1xbxd[:, :, :region_start].shape[-1] != 0:
                x_1xbxd.grad[:, :, :region_start] *= 0
            if x_1xbxd[:, :, region_end:].shape[-1] != 0:
                x_1xbxd.grad[:, :, region_end:] *= 0
    ...
    def _compute_recurrent_jacobians(self, fps):
        ...
        def efficient_jacobian(x_bxd, inputs_bxd):
            ...
            def forward_fn(x_bxd):
                ...
                inhib_stim = torch.zeros_like(x_1xbxd) # Used locally--ugly but necessary.

                F_x_1xbxd, _ = self.rnn(inputs_bx1xd, x_1xbxd, inhib_stim)
