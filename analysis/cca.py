...
rnn_fr = np.load("results/lick_ramp19000_fr.npy")
alm_fr = loadmat("data/alm_fr_population_1s.mat")['fr_population_1s']
striatum_fr = loadmat("data/striatum_fr_population_1s.mat")['fr_population_1s']

print(rnn_fr.shape) 
#First filter the agent's activity with 20ms gaussian as done with experimental activity during preprocessing
A_agent = gaussian_filter1d(rnn_fr, sigma=20, axis=0)
print(A_agent.shape)
...
A_exp = PC_exp.fit_transform(striatum_fr)[:A_agent.shape[0], :]
